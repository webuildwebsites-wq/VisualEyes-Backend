...
export const getAllSalesPersons = async (req, res) => {
  try {    
    ...
    const salesPersons = await employeeSchema.find(filter).select('username employeeName email phone Department EmployeeType region lab isActive');
...
export const getSalesPersonById = async (req, res) => {
  try {
    ...
    const salesPerson = await employeeSchema.findOne({ 
        ...
        'Department.name': 'SALES',
        EmployeeType: { $in: ['EMPLOYEE', 'SUPERVISOR'] }
      }).select('username employeeName email phone Department EmployeeType region lab isActive profile');
